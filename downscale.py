...
from flowpy import flow_read, flow_write

#import common

# User options
root = './data/FlyingChairs_release/data'
out = './data/downscaled/'
factor = 0.25
num_train = 5000
num_valid = 500
...
TRAIN_INDICES = [i for i in range(1, 22873) if i not in VALIDATE_INDICES]

def downsample2d(inputs, factor):
    _, h, w = inputs.size()
    ho = int(h * factor)
    wo = int(w * factor)
    return tf.adaptive_avg_pool2d(inputs, [ho, wo])

def downsample_flow(flow, factor):
    _, h, w = flow.size()
    ho = int(h * factor)
    wo = int(w * factor)
    resized_flow = tf.adaptive_avg_pool2d(flow, [ho, wo])  # downscaling
...
def main():
    for i in range(1, num_train + num_valid + 1):
        # Training data
        if i <= num_train:
            id = TRAIN_INDICES[i-1]
            print('Train: ', i)
        else:
            id = VALIDATE_INDICES[i-num_train-1]
            print('Valid: ', i)

        img1_np = imread(os.path.join(root, ('%05d' % id) + '_img1.ppm'))
        img2_np = imread(os.path.join(root, ('%05d' % id) + '_img2.ppm'))
        flo_np = flow_read(os.path.join(root, ('%05d' % id) + '_flow.flo'))

        # To torch
        img1 = numpy2torch(img1_np)
        img2 = numpy2torch(img2_np)
        flo = numpy2torch(flo_np)

        # Resize
        img1 = downsample2d(img1, factor)
        img2 = downsample2d(img2, factor)
        flo = downsample_flow(flo, factor)

        # To numpy
        img1_np = torch2numpy(img1)
        img2_np = torch2numpy(img2)
        flo_np = torch2numpy(flo)

        # Save
        imwrite(os.path.join(out, ('%05d' % i) + '_img1.ppm'), np.uint8(img1_np))
        imwrite(os.path.join(out, ('%05d' % i) + '_img2.ppm'), np.uint8(img2_np))
        flow_write(os.path.join(out, ('%05d' % i) + '_flow.flo'), flo_np)
